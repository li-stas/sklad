...
/*****************************************************************
 
 FUNCTION:
 АВТОР..ДАТА..........С. Литовка  12-03-20 * 01:47:07pm
 НАЗНАЧЕНИЕ.........
 ПАРАМЕТРЫ..........
 ВОЗВР. ЗНАЧЕНИЕ....
 ПРИМЕЧАНИЯ.........
 */
static function sum61r(cL)
  local nSum := 0
  If !empty(cL) .and. '":' $ cL
    oSum := JsonDecode(cL)
    //outlog(__FILE__,__LINE__,oSum['Коригування'], oSum['Коригування'] * (-1))
    nSum := oSum['Коригування']
  EndIf

  nSum += getfield('t1', 'ttnr,61', 'rs3', 'ssf')
